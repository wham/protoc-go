...
18. [DONE] Fix `346_enum_subfield_merge` — Enum-level sub-field options (e.g., `option (enum_cfg).label = "tracker"; option (enum_cfg).priority = 10;`) were not being merged in `proto_file`. Added sub-field tracking to `resolveCustomEnumOptions`, added `mergeEnumOptions` and `mergeEnumOptionsInMessages` functions, updated `cloneWithMergedExtUnknowns` to handle EnumOptions, and added `CustomEnumOptions` to `hasSubFieldCustomOpts`. All 3180/3180 tests pass.
